...
#------------------------------------Lines 998 and 999 are for printing alt and groundspeed in main loop---
#---------ARMING COMMAND: self.master.arducopter_arm() 
...
#socketIO = SocketIO('localhost', 8010)
#socketIO2 = SocketIO('localhost', 8011)
...
def cmd_alt(args):
    '''show altitude'''
    print ("GROUNDSPEED: %.1f" % mpstate.status.msgs['VFR_HUD'].groundspeed)
    print ("Alt:  %.1f" % mpstate.status.altitude)
...
def main_loop():
    ...
    while True:
        ...
        #socketIO.emit('altitude msg', mpstate.status.speed)
        #socketIO.emit('altitude msg', mpstate.status.altitude)
        ...
        #socketIO.emit('altitude msg', mpstatus.status.msg['VFR_HUD'].groundspeed)
        
        #socketIO.wait(0.00001)

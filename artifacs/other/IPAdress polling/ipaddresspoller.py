...
# Get data from ifconfig ppp0 and input to ifconfigDev.txt, do regex on it so only the ip address exists and output to ipaddress.json. 
# If device ppp0 does not exist after the script has been running for the specified 
# amount of seconds according to the first time.sleep() the script will go to error: and stop it self.
...
while(True):
 ...
 #os.system("ifconfig wlan0 > ifconfigDev.txt 2> /dev/null")
 #time.sleep (5)      

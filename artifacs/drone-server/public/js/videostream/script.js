...
function reload_img () {
  if(!halted) mjpeg_img.src = "php/videostream/cam_pic.php?time=" + new Date().getTime();
  else setTimeout("reload_img()", 500);
}

function error_img () {
  setTimeout("mjpeg_img.src = 'php/videostream/cam_pic.php?time=' + new Date().getTime();", 100);
}
...
function reload_ajax (last) {
  ajax_status.open("GET","php/videostream/status_mjpeg.php?last=" + last,true);
  ajax_status.send();
}
...
function send_cmd (cmd) {
  ajax_cmd.open("GET","php/videostream/cmd_pipe.php?cmd=" + cmd,true);
  ajax_cmd.send();
}
